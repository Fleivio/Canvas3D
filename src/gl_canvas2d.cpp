...
void CV::line(Vector p1, Vector p2)
{
...
void CV::line(vector<Vector> points){
   glBegin(GL_LINES);
   Vector prev = *(points.begin());

   for(int i = 1; i < (int) points.size() - 1; i++){
      Vector act = points.at(i);
      glVertex2d(prev.x, prev.y);
      glVertex2d(act.x, act.y);
      prev = act;
   }
   glEnd();
}
...
void CV::rect(Vector p1, Vector p2)
{
...
void CV::rectFill(Vector p1, Vector p2)
{
...
void CV::polygon(Vector vec_list[], int elems)
{
   int cont;
   glBegin(GL_LINE_LOOP);
   for (cont = 0; cont < elems; cont++)
   {
      glVertex2d(vec_list[cont].x, vec_list[cont].y);
   }
   glEnd();
}
...
void CV::polygonFill(Vector vec_list[], int elems)
{
   int cont;
   glBegin(GL_POLYGON);
   for (cont = 0; cont < elems; cont++)
...
void CV::small_text(Vector pos, const char *t){
   CV::small_text(pos.x, pos.y, t);
}


void CV::text(Vector pos, const char *t)
{
...
void CV::circle(Vector pos, float radius, int div)
{
...
void CV::circleFill(Vector pos, float radius, int div)
{
...
void CV::translate(Vector offset)
{
   glMatrixMode(GL_MODELVIEW);
   glLoadIdentity();
